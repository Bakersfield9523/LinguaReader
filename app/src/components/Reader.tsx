...
export function Reader({
  ...
  // 全书共享的内嵌字体 @font-face CSS（data: URL 内联），用于恢复书籍自带字体渲染
  const [bookFontCss, setBookFontCss] = useState<string>('');
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed && typeof parsed === 'object') {
          return { ...parsed, fontFamily: normalizeFontToken(parsed.fontFamily) };
        }
      }
    } catch { /* ignore */ }
    return {
  ...
  // 字体解析：auto = 跟随书籍（仅继承兜底），serif/sans-serif = 显式覆盖
  const fontStack = useMemo(
    () => resolveFontStack(settings.fontFamily, book.language),
    [settings.fontFamily, book.language]
  );
  const fontOverride = settings.fontFamily !== 'auto';
  ...
  useEffect(() => {
    ...
    async function loadBook() {
      ...
      try {
        ...
        const { parser, chapters: loadedChapters, contents: loadedContents, cssContents: loadedCss, fontCss: loadedFontCss } = await getBookContent(book);
        ...
        // 加载当前章节内容
        // 章节数超过预加载上限时，loadedContents 里可能没有目标章节，
        // 这里必须按需加载，否则打开书就是空白页。
        const ensureChapterContent = async (idx: number) => {
          const ch = flat[idx];
          if (!ch) return;
          if (loadedContents.has(ch.id)) {
            setChapterContent(loadedContents.get(ch.id) || '');
            // 关键修复：初始章节也要同步注入该章节的书籍 CSS，
            // 否则打开书看到的第一个章节完全没有书籍样式（角标/引用/首字下沉等排版全部丢失）。
            setBookCss(loadedCss.get(ch.id) || '');
            return;
          }
          if (parser instanceof EPUBParser && ch.href) {
            const c = await parser.getChapterContent(ch.href);
            if (!isMounted) return;
            setChapterContent(c.html);
            setBookCss(c.css);
            loadedContents.set(ch.id, c.html);
            if (c.css) loadedCss.set(ch.id, c.css);
          }
        };

        if (initialChapter < flat.length && flat[initialChapter]) {
          await ensureChapterContent(initialChapter);
        } else if (flat.length > 0) {
          // 如果当前章节不存在，加载第一章
          await ensureChapterContent(0);
          setCurrentChapter(0);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    iframeApplySettings(doc, { fontSize: settings.fontSize, lineHeight: settings.lineHeight, theme: settings.theme, fontFamily: fontStack, fontOverride });
  }, [settings.fontSize, settings.lineHeight, settings.theme, fontStack, fontOverride]);
  ...
  return (
    <div
      ref={htmlContentRef}
      className={`reader-html-content reader-theme-${settings.theme}${fontOverride ? ' reader-font-override' : ''}`}
      style={{
    ...
    >
      {bookFontCss ? <style>{bookFontCss}</style> : null}
  ...
  }, [book.format, epubRenderMode, chapterContent, bookCss, bookFontCss]);
        ...
        {showSettings && (
          <div className="border-t border-current/10 p-4">
            <div className="flex items-center gap-6 flex-wrap justify-center">
              ...
              {/* Font */}
              <div className="flex items-center gap-2">
                <span className="text-sm opacity-80">字体</span>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="auto">自动（跟随书籍）</SelectItem>
                    <SelectItem value="serif">衬线 Serif</SelectItem>
                    <SelectItem value="sans-serif">无衬线 Sans</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
...
.reader-html-content {
  ...
  max-width: 44rem;
  margin: 0 auto;
  /* 默认继承兜底：auto 档跟随书籍自带字体；显式选择时由下方 .reader-font-override 覆盖 */
  font-family: var(--reader-font-family, Georgia, 'Times New Roman', serif);
  word-wrap: break-word;
  overflow-wrap: break-word;
}
/* 仅在用户显式选择字体时启用覆盖；排除等宽与图标/首字下沉类，避免破坏原书设计 */
.reader-html-content.reader-font-override,
.reader-html-content.reader-font-override *:not(code):not(pre):not(kbd):not(samp):not(var)
:not([class*="icon"]):not([class*="Icon"]):not([class*="dropcap"]):not([class*="initial"]) {
...
function iframeApplySettings(doc: Document, s: IframeSettings) {
  ...
  doc.documentElement.style.setProperty('--reader-font-family', s.fontFamily || '');
  doc.body.className = 'reader-html-content reader-theme-' + s.theme + (s.fontOverride ? ' reader-font-override' : '');
}
...
function iframeApplyHighlights(doc: Document, list: IframeHighlight[]) {
  ...
  for (const it of items) {
    ...
    // 多个文本节点 —— 旧实现只在单个节点内精确 indexOf，多词高亮/下划线永远匹配不上。
    ...
    for (let h = hits.length - 1; h >= 0; h--) {
      ...
      for (let i = ei; i >= si; i--) {
        const node = nodes[i];
        const len: number = node.nodeValue.length;
...
function buildSrcDoc(
  content: string,
  bookCss: string,
  settings: { fontSize: number; lineHeight: number; theme: string; fontFamily: string; fontOverride: boolean },
  fontCss: string,
): string {
  const themeClass = `reader-theme-${settings.theme}`;
  // 顺序：:root 变量 → 内嵌字体 @font-face → 书籍 CSS → 阅读器基础 CSS
  return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>:root{--reader-font-family:${settings.fontFamily || ''}}</style>
<style>${fontCss || ''}</style>
<style>${bookCss || ''}</style>
<style>${READER_BASE_CSS}</style>
</head>
<body class="reader-html-content ${themeClass}${settings.fontOverride ? ' reader-font-override' : ''}">
${content}
</body>
</html>`;
}
