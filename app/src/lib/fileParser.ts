...
import { FONT_STACKS, GENERIC_FAMILY_RE } from '@/lib/fonts';
...
export class EPUBParser {
  ...
  // 内嵌字体（独立于 resources，避免污染 getCoverUrl 封面识别策略4/5 与松匹配）
  private fontResources: Map<string, string> = new Map(); // href -> data URL
  private fontFaceRules: Map<string, string> = new Map(); // dedupeKey -> @font-face 原文
  private fontCssCache: string | null = null; // getFontCss 缓存（harvest 后失效）
  private lang: Language = 'en';
  private spineHrefSet: Set<string> | null = null; // spine 内所有章节 href（相对 OPF 目录）
  private spineKeyMap: Map<string, string> | null = null; // canonicalKey -> spine 规范 href
  private hrefResolveCache: Map<string, string | null> = new Map(); // 目录 href -> spine href
  ...
  // ============ 目录 href → spine href 解析 ============
  // epub.js 的 navigation.toc 直接照搬 nav.xhtml / toc.ncx 里 <a href> 的原文，**不做任何路径解析**；
  // 而 spine 的 href 是「相对 OPF 目录」的路径。当 nav 文档不在 OPF 根目录时
  // （典型：OEBPS/Text/nav.xhtml，其中 href 写成 "book-part1.xhtml"，而 spine 里是 "Text/book-part1.xhtml"），
  // 两者基准不同 → spine.get(href) 恒返回 null → 目录齐全但每一章都是空白页。
  // 这里按 nav/ncx 所在目录做标准相对路径解析，并逐级兜底，保证任何目录结构都能命中 spine。

  /**
   * 归一化键：去掉查询串/锚点、统一斜杠、URL 解码、转小写。
   * 同一个文档在 zip / spine / 目录里可能有编码差异（%20 vs 空格）与大小写差异，
   * 不去重会把同一章节当成两个，从而生成重复章节。
   */
  private canonicalKey(p: string): string {
    let s = String(p || '').replace(/\\/g, '/').split('?')[0].split('#')[0];
    try { s = decodeURIComponent(s); } catch { /* ignore */ }
    return s.toLowerCase();
  }

  /** spine 中所有章节 href（每个文档只保留一个规范 href） */
  private getSpineHrefs(): Set<string> {
    if (this.spineHrefSet) return this.spineHrefSet;
    ...
    this.spineHrefSet = set;
    return set;
  }

  /** canonicalKey -> spine 规范 href（用于去重匹配，结果缓存） */
  private spineKeys(): Map<string, string> {
    if (this.spineKeyMap) return this.spineKeyMap;
    const m = new Map<string, string>();
    for (const h of this.getSpineHrefs()) {
      const k = this.canonicalKey(h);
  ...
  /** 目录文档（nav.xhtml / toc.ncx）所在目录，作为 href 的相对解析基准 */
  private getTocBaseDirs(): string[] {
    const dirs: string[] = [];
    const add = (p?: string | false) => {
      if (!p) return;
      const normalized = String(p).replace(/\\/g, '/').split('?')[0].split('#')[0];
      const idx = normalized.lastIndexOf('/');
      const d = idx >= 0 ? normalized.slice(0, idx + 1) : '';
      if (!dirs.includes(d)) dirs.push(d);
    };
    add(this.book?.packaging?.navPath);
    add(this.book?.packaging?.ncxPath);
    if (!dirs.includes('')) dirs.push('');
    return dirs;
  }

  /** 生成一个 href 的所有可能形态（按命中概率排序） */
  private hrefCandidates(rawHref: string): string[] {
    const clean = (rawHref || '').replace(/\\/g, '/').split('?')[0].split('#')[0];
    if (!clean) return [];
    const out: string[] = [];
    const push = (v: string) => {
      if (!v || out.includes(v)) return;
      // 纯数字的 href 会被 epub.js 的 spine.get 误当成章节序号，必须排除
      if (/^\d+$/.test(v)) return;
      out.push(v);
    };
    push(clean);
    try { push(decodeURIComponent(clean)); } catch { /* ignore */ }
    for (const d of this.getTocBaseDirs()) {
      if (!d) continue;
      push(normalizeBookPath(d + clean));
      try { push(normalizeBookPath(d + decodeURIComponent(clean))); } catch { /* ignore */ }
    }
    return out;
  }

  /** 返回能命中 spine 的 href（相对 OPF 目录），解析失败返回 null */
  private resolveHrefToSpine(rawHref: string): string | null {
    const key = (rawHref || '').split('?')[0].split('#')[0];
    if (!key) return null;
    if (this.hrefResolveCache.has(key)) return this.hrefResolveCache.get(key)!;

    const keys = this.spineKeys();
    const cands = this.hrefCandidates(key);
    let hit: string | null = null;

    // 1) 归一化后直接命中
    for (const c of cands) {
      const k = this.canonicalKey(c);
      if (keys.has(k)) { hit = keys.get(k)!; break; }
    }
    ...
    if (!hit) {
      ...
      for (const c of cands) {
        const m = entries.find(([k]) => k.endsWith('/' + this.canonicalKey(c)));
        if (m) { hit = m[1]; break; }
      }
    }
    ...
    // 3) basename 兜底
    if (!hit) {
      const base = this.canonicalKey(key).split('/').pop();
      if (base) {
        for (const [k, v] of keys) {
          if (k.split('/').pop() === base) { hit = v; break; }
        }
      }
    }

    this.hrefResolveCache.set(key, hit);
    if (!hit) console.warn('[EPUB] 目录 href 无法解析到 spine:', rawHref);
    return hit;
  }
  ...
  async extractResources(): Promise<void> {
    try {
      ...
      // 提取内嵌字体文件（.ttf/.otf/.woff/.woff2），用于恢复书籍自带字体渲染
      // （很多 EPUB 自带字体，但 srcdoc 无 base URL 导致 @font-face 失效，落到中文 Windows 的宋体）
      const fontFiles = allFiles.filter(name => /\.(ttf|otf|woff2?)$/i.test(name));
      const fontMime: Record<string, string> = {
        ttf: 'font/ttf', otf: 'font/otf', woff: 'font/woff', woff2: 'font/woff2',
      };
      const FONT_MAX_SINGLE = 2 * 1024 * 1024; // 单文件 ≤ 2MB
      const FONT_MAX_TOTAL = 6 * 1024 * 1024;  // 全书合计 ≤ 6MB（避免内存膨胀）
      let fontTotal = 0;

      for (const path of fontFiles) {
        if (fontTotal >= FONT_MAX_TOTAL) {
          console.warn('[EPUB] 字体总量超限(6MB)，跳过剩余字体');
          break;
        }
        try {
          const zipEntry = archive.zip.files[path];
          if (!zipEntry) continue;
          const arrayBuffer = await zipEntry.async('arraybuffer');
          if (arrayBuffer.byteLength > FONT_MAX_SINGLE) {
            console.warn(`[EPUB] 字体过大跳过 ${path} (${(arrayBuffer.byteLength / 1024 / 1024).toFixed(1)}MB)`);
            continue;
          }
          const ext = path.split('.').pop()?.toLowerCase() || 'ttf';
          const mime = fontMime[ext] || 'font/ttf';
          const dataUrl = arrayBufferToDataUrl(arrayBuffer, mime);
          fontTotal += arrayBuffer.byteLength;

          // 与图片同套规范化键，提升 @font-face url() 命中率
          const storeFont = (p: string) => { if (p) this.fontResources.set(p, dataUrl); };
          const opfDir = (this.book?.directory || '').replace(/\\/g, '/').replace(/\/$/, '');
          const relOpf = opfRelPath(path, opfDir);
          storeFont(path);
          storeFont(path.toLowerCase());
          const fileName = path.split('/').pop()!;
          storeFont(fileName);
          storeFont(fileName.toLowerCase());
          if (relOpf && relOpf !== path) { storeFont(relOpf); storeFont(relOpf.toLowerCase()); }
          try {
            const decodedPath = decodeURIComponent(path);
            storeFont(decodedPath);
            storeFont(decodedPath.toLowerCase());
            const decodedFile = decodedPath.split('/').pop()!;
            storeFont(decodedFile);
            storeFont(decodedFile.toLowerCase());
          } catch { /* ignore */ }
        } catch (e) {
          console.warn('[EPUB] 提取字体失败:', path, e);
        }
      }
      if (fontFiles.length > 0) {
        console.log(`[EPUB] Found ${fontFiles.length} embedded fonts, ${this.fontResources.size} font resource entries`);
      }

      // 预收获：从已提取的 cssResources 中提前收集 @font-face（即便章节尚未处理也能拿到全书字体）
      for (const css of this.cssResources.values()) {
        this.harvestFontFaces(css);
      }
      ...
      if (navigation && navigation.toc && navigation.toc.length > 0) {
        ...
        const buildTree = (items: any[], level: number): Chapter[] => {
          ...
          for (const item of items) {
            ...
            // 目录里的 href 是相对 nav/ncx 文档的，必须解析成 spine href 才能取到正文
            const rawHref: string = item.href || '';
            const href = rawHref ? (this.resolveHrefToSpine(rawHref) || rawHref) : '';
            const ch: Chapter = {
              id: href || `ch-${globalIdx}`,
        ...
        if (treeChapters.length > 0) {
          return await this.appendMissingSpineChapters(treeChapters, globalIdx);
        }
      }
  ...
  /**
   * 目录不完整时的兜底：把 spine 中**没有被任何目录条目覆盖**的文档补成章节。
   *
   * 背景：不少 EPUB（尤其 calibre 早期版本转出的老书）目录只有一条 "Start"，
   * 正文全部躺在 spine 里却不在目录中。只按目录建章节 → 整本书"只剩封面"。
   * 以 spine 为基准补全可以保证任何一本书的正文都不会丢。
   */
  private async appendMissingSpineChapters(chapters: Chapter[], startIndex: number): Promise<Chapter[]> {
    try {
      const spineHrefs = Array.from(this.getSpineHrefs());
      if (spineHrefs.length === 0) return chapters;

      // 只统计「能解析到 spine」的目录条目，避免同一文档被判定为缺失而重复补章节
      const covered = new Set<string>();
      const collect = (list: Chapter[]) => {
        for (const c of list) {
          if (c.href && this.resolveHrefToSpine(c.href)) {
            covered.add(this.canonicalKey(c.href));
          }
      ...
      const missing = spineHrefs.filter(h => !!h && !covered.has(this.canonicalKey(h)));

      if (missing.length === 0) return chapters;

      // 上限保护：极端情况（每页一个 spine 文件的大部头）不无限补章节
      const MAX_APPEND = 1000;
      // 取标题要解压文档，只给前若干个取名，避免导入变慢
      const MAX_TITLE_LOOKUP = 60;
      const list = missing.slice(0, MAX_APPEND);
      console.log(`[EPUB] 目录缺失 ${missing.length} 个 spine 文档，正在补全为章节`);

      let idx = startIndex;
      const seenTitles = new Set<string>();
      for (const ch of flattenChapters(chapters)) seenTitles.add(ch.title.trim().toLowerCase());
      for (const ch of chapters) seenTitles.add(ch.title.trim().toLowerCase());

      for (const href of list) {
        const fallback = `Section ${idx + 1}`;
        let title = fallback;
        if (idx - startIndex < MAX_TITLE_LOOKUP) {
          title = await this.deriveChapterTitle(href, fallback);
        }
        // 重名去重，避免目录里出现一串同名条目
        let finalTitle = title;
        let n = 2;
        while (seenTitles.has(finalTitle.trim().toLowerCase())) {
          finalTitle = `${title} (${n++})`;
        }
        seenTitles.add(finalTitle.trim().toLowerCase());
      ...
      console.warn('[EPUB] 补全缺失章节失败:', e);
      return chapters;
    }
  }

  /** 按「相对 OPF 目录」的 href 读取 zip 内原文（用于给目录缺失的章节取标题） */
  private async readZipText(href: string): Promise<string | null> {
    try {
      const archive = this.book?.archive;
      ...
      const dir = (this.book?.directory || '').replace(/\\/g, '/').replace(/^\.\//, '').replace(/\/+$/, '');
      const direct = [key, dir ? dir + '/' + key : ''].filter(Boolean);
      for (const c of direct) {
        const e = files[c] || files[c.toLowerCase()];
      ...
      for (const k of Object.keys(files)) {
        ...
        if (lk === lower || lk.endsWith('/' + lower)) return await files[k].async('string');
      }
    } catch {
      // ignore
    }
  ...
  /** 从章节文档里取一个可读标题：<h1>~<h4> 优先，其次 <title> */
  private async deriveChapterTitle(href: string, fallback: string): Promise<string> {
    ...
    try {
      const raw = await this.readZipText(href);
      if (!raw) return fallback;
      const head = raw.length > 400000 ? raw.slice(0, 400000) : raw;

      const h = head.match(/<h[1-4][^>]*>([\s\S]*?)<\/h[1-4]>/i);
      if (h) {
        const t = decodeEntities(h[1]);
        if (t && t.length > 0 && t.length <= 120) return t;
      }
      const t = head.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
      if (t) {
        const s = decodeEntities(t[1]);
  ...
  private async loadRawContent(href: string): Promise<{ html: string; css: string }> {
    try {
      // 目录 href 与 spine href 基准可能不同，逐个候选尝试，直到 spine.get 命中
      const resolved = this.resolveHrefToSpine(href);
      const ordered = resolved
        ? [resolved, ...this.hrefCandidates(href).filter(c => c !== resolved)]
        : this.hrefCandidates(href);

      let item: any = null;
      for (const cand of ordered) {
        try {
          const got = await this.book.spine.get(cand);
          if (got) { item = got; break; }
        } catch {
          // ignore，继续下一个候选
        }
      }
      ...
      if (!item) {
        try {
          const section = await this.book.section?.(resolved || href);
          if (section) {
            const html = section.output ? section.output() : (section.textContent || '');
            return this.processHTML(html, resolved || href);
          }
        } catch (e) {
          // ignore
        }
        console.warn('[EPUB] 章节内容加载失败（所有候选路径均未命中 spine）:', href);
        return { html: '', css: '' };
      }

      await item.load(this.book.load.bind(this.book));

      // 图片/CSS 相对路径要按「章节真实所在的目录」解析，因此用解析后的 href 作为 base
      const baseHref = resolved || item.href || href;
      ...
      if (!html) {
        ...
        try {
          ...
          if (frag) {
            ...
            return this.processHTML(serializer.serializeToString(frag), baseHref);
          }
        } catch {
          // ignore
        }
      ...
      return this.processHTML(html, baseHref);
    } catch (e) {
  ...
  private processHTML(html: string, baseHref?: string): { html: string; css: string } {
    ...
    if (bookCss) {
      bookCss = this.harvestFontFaces(bookCss); // 剥离 @font-face（避免每章带一份），存入全书记规则
      bookCss = this.withLatinFallback(bookCss); // 把裸通用族替换为拉丁字体栈，避免落到中文宋体
      bookCss = this.replaceResourceUrls(bookCss, baseHref); // 图片等 url() -> base64
    }
  ...
  /**
   * 从 CSS 文本中摘出所有 @font-face 块，存入 fontFaceRules（按 family|src 去重），
   * 并从 CSS 中删除（避免 120 章各存一份 MB 级 base64，≈560MB 内存）。
   * 返回去掉 @font-face 后的 CSS。
   */
  private harvestFontFaces(css: string): string {
    const re = /@font-face\s*\{[^}]*\}/gi;
    let m: RegExpExecArray | null;
    while ((m = re.exec(css))) {
      const block = m[0];
      const key = this.fontFaceKey(block);
      if (!this.fontFaceRules.has(key)) {
        this.fontFaceRules.set(key, block);
        this.fontCssCache = null; // 缓存失效
      }
    }
    return css.replace(re, '');
  }

  private fontFaceKey(block: string): string {
    const fam = (block.match(/font-family\s*:\s*([^;{}]+);/i) || [])[1] || '';
    const src = (block.match(/src\s*:\s*([^;{}]+);/i) || [])[1] || '';
    return (fam + '|' + src).toLowerCase();
  }

  /**
   * 严格匹配字体 @font-face 的 src（与 findResourceBase64 不同，不复用 includes 双向松匹配，
   * 避免把图片资源误当字体）。
   */
  private findFontBase64(src: string, baseHref?: string): string | null {
    if (!src) return null;
    ...
    for (const c of candidates) {
      if (this.fontResources.has(c)) return this.fontResources.get(c)!;
    }
    return null;
  }

  /**
   * 把书籍 CSS 里 font-family 列表的「最后一项为通用族」替换为语言字体栈（保留前面的具体族名，
   * 如 'PT-Serif'）；没有通用族结尾则在末尾追加语言栈作为兜底。等宽字体保住 code/pre 外观。
   * 跳过含 var() 的声明（避免破坏 --reader-font-family 变量）。
   */
  private withLatinFallback(css: string): string {
    const stack = this.lang === 'ja' ? FONT_STACKS.jaSerif : FONT_STACKS.serif;
    const monoStack = FONT_STACKS.mono;
    return css.replace(/font-family\s*:\s*([^;}{]+);/gi, (match, body: string) => {
      if (/var\(/i.test(body)) return match; // 跳过 CSS 变量
      const families = body.split(',').map(s => s.trim()).filter(Boolean);
      if (families.length === 0) return match;
      const last = families[families.length - 1];
      if (GENERIC_FAMILY_RE.test(last)) {
        if (/monospace/i.test(last)) {
          families[families.length - 1] = monoStack; // 等宽：保住 code/pre
        } else {
          families[families.length - 1] = stack;     // serif/sans-serif/... -> 拉丁栈
        }
        return `font-family: ${families.join(', ')};`;
      }
      // 没有通用族结尾：追加语言栈作为兜底
      return `font-family: ${families.join(', ')}, ${stack};`;
    });
  }

  /**
   * 返回全书共享的 @font-face CSS（url 已内联为 data: URL）。
   * 只保留 src 能解析到字体文件的规则；解析不到的规则丢弃。
   * 缓存失效由 harvestFontFaces 触发（fontCssCache = null）。
   */
  getFontCss(): string {
    if (this.fontCssCache !== null) return this.fontCssCache;
    const parts: string[] = [];
    for (const block of this.fontFaceRules.values()) {
      let hasData = false;
      const replaced = block.replace(/url\(['"]?([^'"\)]+)['"]?\)/g, (mm, url: string) => {
        const data = this.findFontBase64(url);
        if (data) { hasData = true; return `url("${data}")`; }
        return mm;
      });
      if (hasData) parts.push(replaced);
    }
    this.fontCssCache = parts.join('\n');
...
export async function getBookContent(
  book: Book
): Promise<{ parser: EPUBParser | PDFParser | null; chapters: Chapter[]; contents: Map<string, string>; cssContents: Map<string, string>; fontCss: string }> {
  const contents = new Map<string, string>();
  ...
  try {
    ...
    if (book.format === 'epub') {
      ...
      try {
        parser = new EPUBParser(arrayBuffer);
        await parser.init();
        parser.setLanguage(book.language);
      } catch (e) {
        return { parser: null, chapters: book.chapters || [], contents, cssContents, fontCss: '' };
      }
      ...
      // 章节极多时（按 spine 补全后可能从几十涨到几百）不再一次性预加载全部正文，
      // 否则打开书籍会长时间无响应。超出的章节由 Reader 在切换/初始化时按需加载。
      ...
      const loadChapterRecursively = async (chapter: Chapter) => {
        if (chapter.href) {
          if (preloaded >= PRELOAD_LIMIT) return;
          try {
      ...
      if (chapters.length > 0) {
        for (const chapter of chapters) {
          ...
          if (preloaded >= PRELOAD_LIMIT) break;
        }
      }

      if (preloaded >= PRELOAD_LIMIT) {
        console.log(`[EPUB] 章节数过多，仅预加载前 ${preloaded} 章，其余按需加载`);
      }

      return { parser, chapters, contents, cssContents, fontCss: parser.getFontCss() };
      ...
      return { parser, chapters, contents, cssContents, fontCss: '' };
      ...
      return { parser: null, chapters, contents, cssContents, fontCss: '' };
    }
  } catch (e) {
